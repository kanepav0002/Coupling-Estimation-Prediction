function run_KRR_repeats_HCP(FC_name, y_name, covars, subject_list, sub_inclusions, outdir, orig_outstem, base_dir)
...
% orig_outstem= string that results will be saved with in the directory
    ...
    krr_directory=sprintf('%s/KRR',base_dir);
    cd(krr_directory)
    addpath(genpath(sprintf('%s/utils/CBIG-master',base_dir)))
    %addpath(genpath('~/kg98_scratch/Kane/utils'))
    
    data_set='HCP';
    data_csv=y_name;
    if strcmp(data_set, "HCP")
        family_csv=('file_list/HCP/restricted_1110.csv');
    else
        family_csv=('file_list/ABCD/Family_IDs.csv');
    end
    RSFC_file=(FC_name);
    num_test_folds=20;
    num_inner_folds=20;
...
%% CV split
    ...
    for rep=1:length(seeds)
        ...
        if strcmp(data_set, 'HCP')
            ...
            CBIG_cross_validation_data_split( subject_list, family_csv, 'src_subject_id', ...
            'rel_family_id', num_test_folds, seed, fullfile(outdir, ['randseed_' num2str(seed)]), ',' );
        end
